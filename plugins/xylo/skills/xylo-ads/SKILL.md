---
name: xylo-ads
description: Manage and analyze Meta, Google, and TikTok ad campaigns through the Xylo MCP server. Use this skill any time the user asks about ad performance (ROAS, CPA, CTR, CPM, conversions), wants to launch/edit/pause/duplicate/scale campaigns, upload or group creative, analyze audience or copy performance, break down an ad creative (hooks, awareness level, psychology, conversion architecture), run an account audit, set up automated rules (auto-pause, auto-scale budget, alerts, dayparting, budget rebalancing) or value-rule bid multipliers, research competitor ads, look up an Instagram handle or monitor UGC/tagged posts, publish or manage Facebook Page and Instagram posts/reels/stories or comments, set up pixel/CAPI/offline-conversion tracking, or compare platforms — including when the user refers to an ad account only by its brand name.
---

# Xylo Ads Skill

This is the agent playbook for the Xylo MCP server. Xylo carries **300+ ad operations across Meta Ads, Google Ads, and TikTok Ads through 25 tools**. Treat this document as the source of truth for how to use them.

**Your actual tool list is the ground truth.** The full surface is 25 tools (18 dispatchers + `describe` + `search_tools` + 4 named AI tools + `send_feedback`). If you see hundreds of platform-prefixed tools instead (`meta_list_campaigns`, …), the user is on an old connection — everything still works, but suggest reconnecting to `https://xylomcp.com/api/mcp` when convenient.

**Staying up to date.** When any tool result begins with a block titled **"⚠️ IMPORTANT: XYLO UPDATE AVAILABLE"**, stop and relay it to the user as a highlighted callout (including the skill download link) before continuing. It means a newer version of the tools and skill is available and the user should re-download the skill and refresh their Xylo connector. Don't bury or skip it.
...
- **Reads:** `accounts` (account discovery + the saved brief), `query` (every entity read), `insights` (every metric/report), `plan` (targeting search + estimates + ad preview), `creative` (AI creative analysis/diagnosis), `competitors` (public ad libraries), `warehouse` (SQL), `knowledge` (reference brains).
...
One connector: **`https://xylomcp.com/api/mcp`** — the complete surface (all platforms, all 25 tools). Platform choice is the `channel` parameter, not a separate URL. Older platform-scoped connections and the previous per-tool surface keep working during the retirement window, but new setups should always use this URL.

## The Universal Workflow

Every task follows the same five steps. Do not skip them.

1. **Resolve the account.** If the user named a brand, call `accounts({channel:"meta"/"google"/"tiktok", action:"list", params:{query:"<brand>"}})` to resolve it to an `ad_account_id` / `customer_id` / `advertiser_id`. Never guess. No `params.query` lists every connected account (the discovery entry point). The Meta list also returns `default_page_id` (→ `page_id`), `default_instagram_id` (→ `instagram_actor_id`), `default_catalog_id` (→ `product_catalog_id`), `warehouse_enabled` + `last_synced_at`, and the org `plan` in response meta. If `query` returns multiple accounts, ask which one — don't assume.
2. **Recall the account's memory.** Once resolved, call `accounts({action:"read_brief", params:{platform, account_id}})` BEFORE planning or writing anything. It returns the freeform `brief` and slug-keyed `memory` facts — naming conventions, default budgets, geo/audience preferences, product sets, brand voice, where new ads go. On multi-account brands it also returns `brand.accounts_knowledge`, the sibling accounts' briefs and memories, so check there before asking for context the brand already taught another account. Honor them; don't make the user repeat context.
3. **Pick the data source (Meta accounts only — Google/TikTok have no warehouse; skip to step 4).** Check `warehouse_enabled` from the accounts list. If `true`, prefer `warehouse({action:"query"})` and the `ad_integrity` lens for slicing-heavy tasks (top N by spend, 90-day trends, every ad linking to a domain). If `false`, use live insights. Free-tier accounts are always live-only.
...
4. `insights({channel:"meta", lens:"winners"})`, the top delivering ads **grouped by optimization goal and ranked by spend within each group**. Defaults to the last 30 days — if that gives you enough winners, stop there. If it looks thin, rewind one 30-day window at a time with `since`/`until` (today Jun 30 → pull May 1–30 next, then Apr 1–30) until you have enough; on huge accounts prefer raising `min_spend` over pulling more windows. Ingest only winners, never the whole account. **Compare only within one goal:** winners from different goals aren't comparable (a purchase ad's ROAS vs a lead ad's cost-per-result are different yardsticks) — work the top-spend group (the account's dominant objective, returned first), and judge it on the metric its `is_value_optimized` flag names: value/purchase goals (`roas > 0`) by **ROAS**, leads/installs/traffic by **cost_per_result**. **Spend is the winner signal, not ROAS on noise:** Meta only keeps feeding budget to ads it trusts to convert, so a scaled ad is a proven winner by definition — a $16 / 2-purchase ad can post a freak ROAS but Meta didn't scale it, so it's noise. On small accounts a sub-floor ad (~$100 spend or <~10 results) can slip in as `low_confidence:true` — treat those as unproven, keep one only as a labeled format example. **Pick at least 5 winners, up to 15 — lean high, more references make later generation better — and get a mix of image and video ads (bias to images when choosing between comparable winners, they're cheaper for the user to regenerate).** Meta-only lens: on Google/TikTok pull the same 30-day windows of ad-level insights, group by goal, and rank on spend yourself. Also profile the delivered audience once — `insights({lens:"account", params:{breakdowns:"age,gender"}})`, reading spend (who's served) AND results (who buys) separately — and save the dominant converting demographic as `audience_profile`.
...
6. `creative({action:"analyze", channel:"meta"})` on the 5–15 winners you selected in step 4 (the image + video mix), then diff the structured fields and draft the shared winning DNA (`creative_winning_dna`) — hold for step 7. Do call `media({action:"save_winning_ad", params:{platform, account_id, ad_id}})` on each analyzed winner right away (measured data, not an assumption; dedupes by creative, attaches 30-day stats and a description). **Multi-media ads** (one ad carrying ≥2 uploaded videos/images — `creative_format:"mixed"` or a media roster of ≥2): the ad winning doesn't mean every asset in it won, so don't save the bundle. Break it down first — `insights({channel:"meta", lens:"ad", params:{ad_id, breakdowns:"video_asset"}})` (or `"image_asset"`) returns per-asset spend/results — then save only the winning asset(s) with `media({action:"save_winning_ad", params:{…, media_index, metrics_snapshot}})`, where `media_index` is the asset's 0-based roster position and `metrics_snapshot` is its own breakdown row (the server keeps it instead of the whole-ad rollup, so the saved entry credits the asset that actually won).
...
6. `media({action:"finalize"})` to push the chosen output to the live Meta ad account, then `create({resource:"ad"})` to build the ad. It lands PAUSED like every other create.
...
- `insights({channel:"cross", lens:"cross_platform"})` *(Meta+Google+TikTok)* — normalized side-by-side view with per-platform and grand totals. **Google conversion lag applies:** when the window touches the last ~7 days, Google conversions are incomplete and the response prepends a ⚠️ note — caveat the Google column or shift the window to end ≥7 days ago (never compare Google ROAS raw against Meta on a recent window).
