...
| Generate ad VIDEO with AI                                        | Generating Creative (AI)       | Read `knowledge({topic:"video_generation"})` first. Kling only animates an approved existing static; Seedance is the default for every net-new video format. Gemini Omni is opt-in and remains the video editor. `generate_video` is async — poll `video_status`. |
