...
**Codex-only image generation override.** Codex has its own GPT Image capability, so default to that for every net-new or edited ad image instead of spending Xylo generation credits. Read the Xylo brand brief, format recipe, and generation guidance as usual; use Xylo to locate exact product/logo/catalog assets, download and visually inspect every candidate before choosing a sufficient reference set, then pass the selected local files to Codex's image-generation or image-editing capability. Save and show the resulting images locally. Do not call Xylo's image `media({action:"generate"})` route unless the user explicitly asks to generate through Xylo. If the user later asks to publish or use a Codex-generated image in an ad, upload or stage only the approved file at that point. This override is image-only: video generation, editing, polling, and stitching still use Xylo. The Xylo image-model and credit guidance below is fallback guidance inside Codex, not the default route.
